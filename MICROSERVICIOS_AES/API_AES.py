...
def cifrar_descifrar(mode, action):
    ...
    return render_template('base.html', info={"action":action, "ruta": nombre}),201


@app.route(base_url+'/AES/<action>/<mode>')
def modo(mode,action):
    print(mode,action)
    return render_template('formulario.html',info={'modo': mode, 'action': action}) 
...
def inicio(action):
    ...
    return render_template('Select_mode.html', action={"action": action})   #return jsonify({"modos": ["CBC", "ECB", "OFB" ]}),201

@app.route(base_url+"/AES")
def hello_networkers():
    return render_template('INICIO.html') #return jsonify({"action": ["cifrar", "descifrar"]}),201
